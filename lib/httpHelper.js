...
var httpHelper = {
     ...
     * @param {String=} options.encode 请求数据的编码格式，如果是utf-8，使用escape编码
    ...
    request: function (options, timeout, data, callback, encoding) {
        ...
        if (options.json) {
            ...
            content = (options.encode && options.encode.toLocaleLowerCase() == 'utf-8') ? qs.stringify(data, null, null, {encodeURIComponent: escape}) : qs.stringify(data);
        }
     ...
     * @param {String} [encoding='utf-8'] 编码格式
     * @param {Object=} header 请求头对象
     * @param {String=} reqEncoding 请求数据的编码格式，如果是utf-8，使用escape编码
